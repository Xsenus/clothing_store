...
public class DatabaseInitializer
{
    ...
    private async Task EnsureDefaultAppSettingsAsync(StoreDbContext db)
    {
        ...
        await EnsureAppSettingExistsAsync(db, "payments_robokassa_enabled", "false");
        await EnsureAppSettingExistsAsync(db, "robokassa_merchant_login", "");
        await EnsureAppSettingExistsAsync(db, "robokassa_password1", "");
        await EnsureAppSettingExistsAsync(db, "robokassa_password2", "");
        await EnsureAppSettingExistsAsync(db, "robokassa_test_password1", "");
        await EnsureAppSettingExistsAsync(db, "robokassa_test_password2", "");
        await EnsureAppSettingExistsAsync(db, "robokassa_test_mode", "true");
        await EnsureAppSettingExistsAsync(db, "robokassa_label_prefix", "RK");
        await EnsureAppSettingExistsAsync(db, "robokassa_payment_timeout_minutes", "60");
        await EnsureAppSettingExistsAsync(db, "robokassa_currency_label", "");
        await EnsureAppSettingExistsAsync(db, "robokassa_payment_methods", "");
        ...
        await EnsureAppSettingExistsAsync(db, "delivery_cdek_enabled", "false");
        await EnsureAppSettingExistsAsync(db, "delivery_cdek_use_test_environment", "true");
        ...
        await EnsureAppSettingExistsAsync(db, "delivery_russian_post_from_postal_code", "630099");
        await EnsureAppSettingExistsAsync(db, "delivery_russian_post_mail_type", "POSTAL_PARCEL");
        await EnsureAppSettingExistsAsync(db, "delivery_russian_post_mail_category", "ORDINARY");
        await EnsureAppSettingExistsAsync(db, "delivery_russian_post_dimension_type", "PACK");
        await EnsureAppSettingExistsAsync(db, "delivery_russian_post_package_length_cm", "30");
        await EnsureAppSettingExistsAsync(db, "delivery_russian_post_package_height_cm", "20");
        await EnsureAppSettingExistsAsync(db, "delivery_russian_post_package_width_cm", "10");
        await EnsureAppSettingExistsAsync(db, "delivery_avito_enabled", "false");
        await EnsureAppSettingExistsAsync(db, "delivery_avito_client_id", "");
        await EnsureAppSettingExistsAsync(db, "delivery_avito_client_secret", "");
        await EnsureAppSettingExistsAsync(db, "delivery_avito_scope", "items:info");
        await EnsureAppSettingExistsAsync(db, "delivery_avito_warehouse_address", "");
        await EnsureAppSettingExistsAsync(db, "delivery_avito_notes", "");
    ...
    private static IReadOnlyDictionary<string, string> GetDefaultHomeTrustSettings()
    {
        ...
        return new Dictionary<string, string>
        {
            ...
            ["home_trust_reviews_json"] = ToJson(new
            {
                ...
                items = new[]
                {
                    new { id = "otzovik-18405429", text = "Заказывала пару вещичек (зипка + тишка). Мне все очень понравилось! доставка относительно быстрая. По размерам все отлично подошло, качество хорошее.", author = "polporer", location = "Россия • 6 июн 2026", source = "Otzovik", sourceUrl = "https://otzovik.com/review_18405429.html", sortOrder = 10, enabled = true },
                    new { id = "otzovik-18396489", text = "Покупаю давно тут еще со времён когда в тг и озоне были вещи, качество нормальное, цены низкие, доставка быстрая;", author = "dianka m", location = "Россия • 3 июн 2026", source = "Otzovik", sourceUrl = "https://otzovik.com/review_18396489.html", sortOrder = 20, enabled = true }
                }
            })
        };
    }
