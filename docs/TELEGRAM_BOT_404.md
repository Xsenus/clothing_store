...
Этот документ — пошаговый runbook для ситуации, когда в админке при нажатии **«Проверить (getMe)»** появляется `Request failed: 404`.
...
1. запущен старый backend-бинарник;
2. backend после деплоя не был перезапущен;
...
1. **Проверьте endpoint напрямую через ваш домен**:
...
- `404` → маршрут не найден в текущем backend (идём дальше по шагам).
- `401` → маршрут есть, но неверный/просроченный admin token.
- `400` → маршрут есть, backend обрабатывает запрос; проверяйте токен/payload.
...
3. **Обновите код и пересоберите backend**:
...
5. **Снова проверьте endpoint через curl** (тот же запрос из шага 1).
